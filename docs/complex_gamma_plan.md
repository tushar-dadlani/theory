...
- **Complex functional equation `GammaC(z+1) = z·GammaC(z)` — reduced to the identity theorem** (`GammaCFE.v`, axiom-clean): the FE is *proven on the positive real axis* (`GammaC_FE_real`, from `GammaC_agree` + `Gam_recur`), the difference `FE_diff := LHS − RHS` is proven *holomorphic on Re>0* (`FE_diff_holo`, via the product/chain rules) and *vanishing on ℝ⁺* (`FE_diff_vanishes_real`), and `GammaC_FE_from_identity` machine-checks that the full FE follows once the identity theorem furnishes `FE_diff ≡ 0`. So the FE is now blocked only by the **identity theorem** (holomorphic on Re>0 + vanishing on ℝ⁺ ⟹ ≡ 0) — the repo has the Cauchy integral formula (`CCauchyFormula`) but not the power-series/uniqueness machinery. The IBP alternative (u-derivative of the Γ kernel — only the z-derivative `dgnkC` exists — + improper boundary analysis) remains the other route.
- **Still open**: full `GammaC z ≠ 0` for all `Re z > 0` (i.e. "Γ has no zeros"). With the FE reduced as above, once the FE and a fundamental-strip nonvanishing are in hand, `Gam_recur`-style strip propagation gives nonvanishing everywhere; but the strip nonvanishing and the identity theorem are both still unbuilt. (`H_gamma` in `ZetaStripConfinement.v:48` still *assumes* nonvanishing on `Re>1`.) The A2 route only needs nonvanishing *along the path* from the real anchor to `¼+iT/2`, a narrower — but still unproven — target.

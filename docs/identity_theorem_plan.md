...
1. **Global continuity of the clamped integral** — `PhiN g Rr c n` continuous in `w` *everywhere*.
   ...
   - **Remaining:** `clampw` continuous (radial retraction — just *continuity*, not nonexpansiveness), then compose: `PhiN(w) = (∮ …)(clampw w)` continuous as (pole-Lipschitz integral)∘(continuous clamp). *(Rocq 9.1 quirk: `ring` rejects `Cpow`/`pow`/`INR`-applied atoms — fresh-var helper lemmas + `generalize`/`clearbody`.)*

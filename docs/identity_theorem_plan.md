...
1. **Global continuity of the clamped integral** — `PhiN g Rr c n` continuous in `w` *everywhere*. ✅ **Core done** (`CCauchyCont.v`): `Cpow_diff_bound` (`|aⁿ−bⁿ| ≤ n·Mⁿ⁻¹·|a−b|`) and `Cinv_pow_diff_bound` (the inverse-power Cauchy kernel is Lipschitz in the pole). **Remaining:** the clamp is nonexpansive (`|clampw w1 − clampw w2| ≤ |w1−w2|`, radial projection onto a disk), then the `pathint_ML` continuity estimate `PhiN(w1)−PhiN(w2) = ∮ g·(kernel diff)·z'` bounded by `L·|w1−w2|` → `PhiN` (Lipschitz, hence) continuous everywhere. *(Rocq 9.1 quirk found: `ring` rejects goals with `Cpow`/`pow`/`INR`-applied atoms — use fresh-variable helper lemmas.)*

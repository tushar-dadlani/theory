...
With the non-center winding done, the **interior-point Cauchy formula is now proved** (conditionally): `CCauchyInterior.cauchy_interior_cond` — `∮_{|z|=R} F/(z−w) = 2πi·F(w)` for `|w|<R`, via `pathint_loop_except` (the removable-quotient loop `∮φ=0`) + `winding_interior`, on the full circle. Axiom-clean, plus the reusable `disk_convex`/`disk_open`. Like `trunc_cauchy` itself, `φ`'s removable properties (continuity through `w`, holomorphy off `w`, local boundedness) are **hypotheses**. **Remaining for B1:** derive those `φ`-properties from `F`'s differentiability at `w` (the removable-singularity theorem under the path-composition `CcontC`) to make the formula unconditional — the one analytic step left before B2 (Cauchy derivative formula) and B3 (Taylor).

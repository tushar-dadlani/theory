...
5. **The walk — a scaled/local-margin construction (`identity_propagate_D` is provably unusable: the ONLY nonempty margin-closed set is all of `C`).** Status:
   ...
   - **Remaining:** `ptcont_path`; re-thread the two disk lemmas with `G_c`; the **segment walk** (`reach`: vertical interpolation `q_i=mkC a (INR i/INR N·Im w)`, `walk_step` via `scaled_disk_zero`, base via `real_disk_zero`); FE assembly (`FE_diff≡0` on `{Re>0}` → `GammaC_FE_from_identity`).

...
1. **Global continuity of the clamped integral** — `PhiN g Rr c n` continuous in `w` *everywhere*.
   ...
   - **Remaining — the one self-contained geometric fact:** `clampw` is continuous. In fact **2-Lipschitz**: `|clampw w1 − clampw w2| ≤ 2·|w1 − w2|` in all 4 branch cases (both-inside `≤ |w1−w2|`; mixed and both-outside `≤ 2|w1−w2|` via `w1/|w1| − w2/|w2| = w1(1/|w1|−1/|w2|) + (w1−w2)/|w2|` and `ρ ≤ |w2|`). This is the *last* piece of brick 2. *(Rocq 9.1: `ring` rejects `Cpow`/`pow`/`INR` atoms — fresh-var helpers + `generalize`/`clearbody`.)*

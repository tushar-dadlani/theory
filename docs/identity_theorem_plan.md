...
5. **The walk — needs a scaled/local-margin construction, NOT `identity_propagate_D` directly.** `{Re>0}` is *not* closed under the `Rr+1` margin (`HUmargin`), and since that margin is always `>1`, no `Rr` fixes it (points with `Re z<1` unreachable). Fix: a center-agnostic **scaled** `identity_at_zero_D` (`H_k(w)=ε^k·fseq k(z0+εw)`, `ε<Re z0/(Rr+1)`) + a re-derived per-step segment walk. Then `FE_diff≡0` on `{Re>0}` → `GammaC_FE_from_identity`.

...
(*  n = 2048 so quadrature <= 1.28e-5.  msum <= 0.003406 against a     *)
...
Lemma pos2048 : (0 < 2048)%nat.
Proof. apply Nat.ltb_lt. vm_compute. reflexivity. Qed.

Lemma h12 : Q2R (13 # 16384) = 13 / 8 / INR 2048.
Proof.
  rewrite (INR_lit 2048 2048) by (vm_compute; reflexivity).
  unfold Q2R; simpl; lra.
Qed.

Lemma msum12_b : msum (gint 12) 0 (13 / 8 / INR 2048) 2048
                <= Q2R (3406 # 1000000).
Proof.
  assert (Hh : 0 <= Q2R (13 # 16384)) by (unfold Q2R; simpl; lra).
  assert (Et : Q2R (12 # 1) = 12) by (unfold Q2R; simpl; lra).
  pose proof chk12 as H.
  destruct (Imsum 46 25 32 14 32 12 30 5 4 46 12 (13 # 16384) 2048)
    as [i |] eqn:E; [ | discriminate ].
  pose proof (Imsum_sound 46 25 32 14 32 12 30 5 4 46 12 (13 # 16384) 2048
                i Hh E) as HC.
...
Proof.
  apply (xir_pos_of 12 (13 / 8) 2048 149 (6 / 100000000)
           pos2048 ltac:(lra) Mfin_12 ET_ub (Q2R (3406 # 1000000)) msum12_b).
  - rewrite (INR_lit 2048 2048) by (vm_compute; reflexivity).

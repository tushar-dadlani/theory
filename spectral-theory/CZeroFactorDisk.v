...
(*                                                                    *)
(*    zero_factor_disk :  F pointwise-continuous, is_Cderiv F z (Fp z)  *)
(*      on Cmod z < R2, 1 < R2, F w = C0 and Cmod w < (R2 - 1) / 2      *)
...
(*  removable quotient  H = rphi F w (Fp w)  (CRemovableExt), which is  *)
...
Theorem zero_factor_disk : forall (F Fp : C -> C) (R2 : R) (w : C),
  1 < R2 ->
  Cmod w < (R2 - 1) / 2 ->
...
Proof.
  intros F Fp R2 w HR2 Hw HFptc HFp HFw.
  assert (HR2pos : 0 < R2) by lra.
  assert (HwR2 : Cmod w < R2) by lra.
  assert (Hdw : is_Cderiv F w (Fp w)) by (apply HFp; exact HwR2).
  set (H := rphi F w (Fp w)).
  exists H.
  (* --- the continuity of H, both flavours (Riemann's quotient) ----- *)
  assert (HHptc : forall z eps, 0 < eps -> exists del, 0 < del /\
             forall z', Cmod (Cminus z' z) < del -> Cmod (Cminus (H z') (H z)) < eps)
    by (intros z eps He; exact (rphi_ptcont_dom F w (Fp w) Hdw HFptc z eps He)).
  assert (HHcc : CcontC H) by exact (rphi_cc_dom F w (Fp w) Hdw HFptc).
  (* --- H is holomorphic on the PUNCTURED disk, for free ------------ *)
  assert (HFholo : forall z, Cmod z < R2 -> z <> w -> exists d, is_Cderiv F z d)
    by (intros z Hz _; exists (Fp z); apply HFp; exact Hz).
  assert (HHoff : forall z, Cmod z < R2 -> z <> w -> exists d, is_Cderiv H z d)
    by (intros z Hz Hzw; exact (rphi_holo_off_dom F w (Fp w) R2 HFholo z Hz Hzw)).
  ...
  - (* the factorisation identity, pure algebra *)
    ...
    + unfold H. rewrite (rphi_off F w (Fp w) z N), HFw.

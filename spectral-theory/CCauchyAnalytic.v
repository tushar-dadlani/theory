...
(* ================================================================= *)
(*  General n: the Cauchy integral oint g/(z-w)^n is holomorphic       *)
(* ================================================================= *)

Section CauchyEstGen.
Variable g : C -> C.
Variable Rr : R.
Variable w0 : C.
Variable n : nat.
...
Definition ddn := Rr - Cmod w0.
Lemma ddn_pos : 0 < ddn. Proof. unfold ddn; lra. Qed.

Lemma arcn_w0_lb : forall u, ddn <= Cmod (Cminus (arc Rr u) w0).
Proof.
  intro u. unfold ddn. eapply Rle_trans; [ | apply Cmod_rev_triangle ].
  rewrite (Cmod_arc Rr u) by lra. lra.
Qed.
Lemma arcn_w0_ne : forall u, Cminus (arc Rr u) w0 <> C0.
Proof.
  intros u Hc. pose proof (arcn_w0_lb u) as H. pose proof ddn_pos.
  rewrite Hc, (proj2 (Cmod0 C0) eq_refl) in H. lra.
Qed.
Lemma arcn_w0h_lb : forall u h, ddn - Cmod h <= Cmod (Cminus (arc Rr u) (Cadd w0 h)).
Proof.
  intros u h. unfold ddn. eapply Rle_trans; [ | apply Cmod_rev_triangle ].
  rewrite (Cmod_arc Rr u) by lra. pose proof (Cmod_triangle w0 h). lra.
Qed.
Lemma arcn_w0h_ne : forall u h, Cmod h < ddn -> Cminus (arc Rr u) (Cadd w0 h) <> C0.
Proof.
  intros u h Hh Hc. pose proof (arcn_w0h_lb u h) as H.
  rewrite Hc, (proj2 (Cmod0 C0) eq_refl) in H. lra.
Qed.

Definition Kwn (w : C) (u : R) : C :=
  Cmul (Cmul (g (arc Rr u)) (Cinv (Cpow (Cminus (arc Rr u) w) n))) (arc' Rr u).
Definition Dn (u : R) : C :=
  Cmul (Cmul (g (arc Rr u))
             (Cmul (RtoC (INR n)) (Cinv (Cpow (Cminus (arc Rr u) w0) (S n))))) (arc' Rr u).
Definition Bremn (h : C) (u : R) : C :=
  Cmul (Cmul (g (arc Rr u)) (Bn (Cminus (arc Rr u) w0) h n)) (arc' Rr u).

Lemma Cpow_cont : forall (base : R -> C) k, Ccont base -> Ccont (fun u => Cpow (base u) k).
Proof.
  intros base k Hb; induction k as [|k IH]; cbn [Cpow].
  - apply Ccont_const.
  - apply Ccont_mul; [ exact Hb | exact IH ].
Qed.

Lemma base_w_cont : forall w, Ccont (fun u => Cminus (arc Rr u) w).
Proof. intro w; apply Ccont_minus; [ exact (Ccont_arc Rr) | apply Ccont_const ]. Qed.

Lemma Kwn_cont : forall w, (forall u, Cminus (arc Rr u) w <> C0) -> Ccont (Kwn w).
Proof.
  intros w Hne; unfold Kwn. apply Ccont_mul; [ | apply Ccont_arc' ].
  apply Ccont_mul; [ exact (Hg (arc Rr) (Ccont_arc Rr)) | ].
  apply Ccont_inv; [ apply Cpow_cont, base_w_cont | intro u; apply Cpow_ne0; exact (Hne u) ].
Qed.

Lemma Dn_cont : Ccont Dn.
Proof.
  unfold Dn. apply Ccont_mul; [ | apply Ccont_arc' ].
  apply Ccont_mul; [ exact (Hg (arc Rr) (Ccont_arc Rr)) | ].
  apply Ccont_mul; [ apply Ccont_const | ].
  apply Ccont_inv; [ apply Cpow_cont, base_w_cont
                   | intro u; apply Cpow_ne0; apply arcn_w0_ne ].
Qed.

Lemma Bremn_cont : forall h, Cmod h < ddn -> Ccont (Bremn h).
Proof.
  intros h Hh; unfold Bremn, Bn. apply Ccont_mul; [ | apply Ccont_arc' ].
  apply Ccont_mul; [ exact (Hg (arc Rr) (Ccont_arc Rr)) | ].
  assert (Hbh : Ccont (fun u => Cminus (Cminus (arc Rr u) w0) h))
    by (apply Ccont_minus; [ apply base_w_cont | apply Ccont_const ]).
  assert (Hne_h : forall u, Cminus (Cminus (arc Rr u) w0) h <> C0)
    by (intro u; replace (Cminus (Cminus (arc Rr u) w0) h) with (Cminus (arc Rr u) (Cadd w0 h)) by ring;
        apply arcn_w0h_ne; exact Hh).
  apply Ccont_sub; [ apply Ccont_sub | ].
  - apply Ccont_inv; [ apply Cpow_cont; exact Hbh | intro u; apply Cpow_ne0; exact (Hne_h u) ].
  - apply Ccont_inv; [ apply Cpow_cont, base_w_cont | intro u; apply Cpow_ne0; apply arcn_w0_ne ].
  - apply Ccont_scal. apply Ccont_scal.
    apply Ccont_inv; [ apply Cpow_cont, base_w_cont | intro u; apply Cpow_ne0; apply arcn_w0_ne ].
Qed.

Lemma Bremn_eq : forall h u,
  Cminus (Cminus (Kwn (Cadd w0 h) u) (Kwn w0 u)) (Cmul h (Dn u)) = Bremn h u.
Proof.
  intros h u. unfold Kwn, Dn, Bremn, Bn.
  replace (Cminus (arc Rr u) (Cadd w0 h)) with (Cminus (Cminus (arc Rr u) w0) h) by ring.
  ring.
Qed.

Lemma cauchy_diff_int_gen : forall h
  (HA : Ccont (Kwn (Cadd w0 h))) (HB : Ccont (Kwn w0)) (HC : Ccont Dn) (HD : Ccont (Bremn h)),
  Cminus (Cminus (Cintf (Kwn (Cadd w0 h)) HA 0 (2 * PI)) (Cintf (Kwn w0) HB 0 (2 * PI)))
         (Cmul h (Cintf Dn HC 0 (2 * PI)))
  = Cintf (Bremn h) HD 0 (2 * PI).
Proof.
  intros h HA HB HC HD.
  assert (Hpi : 0 <= 2 * PI) by (generalize PI_RGT_0; lra).
  assert (Hsub1 : Ccont (fun u => Cminus (Kwn (Cadd w0 h) u) (Kwn w0 u)))
    by (apply Ccont_sub; assumption).
  assert (Hhk2 : Ccont (fun u => Cmul h (Dn u))) by (apply Ccont_scal; exact HC).
  rewrite <- (Cintf_sub (Kwn (Cadd w0 h)) (Kwn w0) HA HB Hsub1 0 (2 * PI) Hpi).
  rewrite <- (Cintf_cmul_l h Dn HC Hhk2 0 (2 * PI) Hpi).
  rewrite <- (Cintf_sub (fun u => Cminus (Kwn (Cadd w0 h) u) (Kwn w0 u))
               (fun u => Cmul h (Dn u)) Hsub1 Hhk2
               (Ccont_sub _ _ Hsub1 Hhk2) 0 (2 * PI) Hpi).
  apply (Cintf_ext _ (Bremn h) (Ccont_sub _ _ Hsub1 Hhk2) HD 0 (2 * PI)).
  intro u. apply Bremn_eq.
Qed.

Theorem cauchy_est_gen : forall (Mg : R), 0 <= Mg -> (forall u, Cmod (g (arc Rr u)) <= Mg) ->
  forall h (HA : Ccont (Kwn (Cadd w0 h))) (HB : Ccont (Kwn w0)) (HC : Ccont Dn)
         (HD : Ccont (Bremn h)),
  Cmod h < ddn / 2 ->
  Cmod (Cminus (Cminus (Cintf (Kwn (Cadd w0 h)) HA 0 (2 * PI)) (Cintf (Kwn w0) HB 0 (2 * PI)))
               (Cmul h (Cintf Dn HC 0 (2 * PI))))
  <= 2 * (Mg * Wn (ddn / 2) ddn n * Rr) * (2 * PI) * (Cmod h * Cmod h).
Proof.
  intros Mg HMg Hgb h HA HB HC HD Hh.
  pose proof ddn_pos as Hdd.
  rewrite (cauchy_diff_int_gen h HA HB HC HD).
  set (M := Mg * (Cmod h * Cmod h * Wn (ddn / 2) ddn n) * Rr).
  assert (HBrem : forall u, 0 <= u <= 2 * PI -> Cmod (Bremn h u) <= M).
  { intros u _. unfold Bremn, M.
    rewrite Cmod_mul, (Cmod_arc' Rr u ltac:(lra)), Cmod_mul.
    apply Rle_trans with (Mg * (Cmod h * Cmod h * Wn (ddn / 2) ddn n) * Rr).
    - apply Rmult_le_compat_r; [ lra | ].
      apply Rmult_le_compat; [ apply Cmod_nonneg | apply Cmod_nonneg | apply Hgb | ].
      apply (bracket_gen_bound n (Cminus (arc Rr u) w0) h (ddn / 2) ddn).
      + apply arcn_w0_ne.
      + replace (Cminus (Cminus (arc Rr u) w0) h) with (Cminus (arc Rr u) (Cadd w0 h)) by ring;
          apply arcn_w0h_ne; lra.
      + lra.
      + replace (Cminus (Cminus (arc Rr u) w0) h) with (Cminus (arc Rr u) (Cadd w0 h)) by ring;
          eapply Rle_trans; [ | apply arcn_w0h_lb ]; lra.
      + exact Hdd.
      + apply arcn_w0_lb.
    - apply Req_le; ring. }
  eapply Rle_trans; [ apply (Cintf_ML (Bremn h) HD 0 (2 * PI) M ltac:(generalize PI_RGT_0; lra) HBrem) | ].
  unfold M. apply Req_le. ring.
Qed.

Theorem cauchy_type_holo_gen :
  forall (Mg : R), 0 <= Mg -> (forall u, Cmod (g (arc Rr u)) <= Mg) ->
  forall (Phi : C -> C) (HDn : Ccont Dn),
  (forall w (Hc : Ccont (Kwn w)), Cmod (Cminus w w0) < ddn / 2 ->
     Phi w = Cintf (Kwn w) Hc 0 (2 * PI)) ->
  is_Cderiv Phi w0 (Cintf Dn HDn 0 (2 * PI)).
Proof.
  intros Mg HMg Hgb Phi HDn Hagree eps Heps.
  pose proof ddn_pos as Hdd.
  set (C := 2 * (Mg * Wn (ddn / 2) ddn n * Rr) * (2 * PI)).
  assert (HWnn : 0 <= Wn (ddn / 2) ddn n) by (apply Wn_nonneg; lra).
  assert (HC0 : 0 <= C).
  { unfold C. apply Rmult_le_pos; [ | generalize PI_RGT_0; lra ].
    apply Rmult_le_pos; [ lra | ].
    apply Rmult_le_pos; [ apply Rmult_le_pos; [ exact HMg | exact HWnn ] | lra ]. }
  exists (Rmin (ddn / 2) (eps / (C + 1))). split.
  { apply Rmin_pos; [ lra | apply Rdiv_lt_0_compat; [ exact Heps | lra ] ]. }
  intros h Hh.
  assert (Hhalf : Cmod h < ddn / 2) by (eapply Rlt_le_trans; [ exact Hh | apply Rmin_l ]).
  assert (Hheps : Cmod h < eps / (C + 1)) by (eapply Rlt_le_trans; [ exact Hh | apply Rmin_r ]).
  assert (HcW0 : Ccont (Kwn w0)) by (apply Kwn_cont; apply arcn_w0_ne).
  assert (HcWh : Ccont (Kwn (Cadd w0 h))) by (apply Kwn_cont; intro u; apply arcn_w0h_ne; lra).
  assert (HcBrem : Ccont (Bremn h)) by (apply Bremn_cont; lra).
  rewrite (Hagree (Cadd w0 h) HcWh
             ltac:(replace (Cminus (Cadd w0 h) w0) with h by ring; exact Hhalf)).
  rewrite (Hagree w0 HcW0
             ltac:(replace (Cminus w0 w0) with C0 by ring;
                   rewrite (proj2 (Cmod0 C0) eq_refl); lra)).
  replace (Cmul (Cintf Dn HDn 0 (2 * PI)) h)
     with (Cmul h (Cintf Dn HDn 0 (2 * PI))) by ring.
  eapply Rle_trans; [ apply (cauchy_est_gen Mg HMg Hgb h HcWh HcW0 HDn HcBrem Hhalf) | ].
  fold C.
  assert (Hmh : 0 <= Cmod h) by apply Cmod_nonneg.
  assert (Hlt : Cmod h * (C + 1) < eps).
  { apply (Rmult_lt_reg_r (/ (C + 1))); [ apply Rinv_0_lt_compat; lra | ].

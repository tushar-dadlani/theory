...
(*  FirstPrimeZeta capstone.  The alternating zeta ("eta") is the sum       *)
(*  that "oscillates by odd and even" (Stage 3's (-2)^inf), and its         *)
(*  closed form pulls out exactly the first prime via the factor 2^{1-s}:  *)
(*                                                                    *)
(*     sum_{n>=1} (-1)^{n-1} n^{-s}  =  (1 - 2^{1-s}) . zeta(s).            *)
(*                                                                    *)
(*  Formalized (real, s>1 for convergence) as a LIMIT identity: if the      *)
(*  zeta series  sum_{n>=1} n^{-s}  converges to Z, then the alternating    *)
(*  partial sums (at even cutoffs) converge to (1 - 2^{1-s}) Z              *)
(*  (`eta_from_zeta`).  The engine is the exact partial-sum identity        *)
(*                                                                    *)
(*    sum_{n=1}^{2(M+1)} (-1)^{n-1} n^{-s}                                  *)
(*      = sum_{n=1}^{2(M+1)} n^{-s}  -  2.2^{-s}.sum_{n=1}^{M+1} n^{-s}     *)
(*                                                                    *)
(*  (`eta_partial_id`, by induction: the even terms are pulled out via      *)
(*  z(2n) = z(2).z(n), z multiplicative, reusing the parity powers          *)
(*  m1_pow_even/m1_pow_odd from CountTwoCollapse), then a limit passage     *)
(*  (subsequence + CV_minus + CV_mult).  `z s n = n^{-s}` is Ell2Zeta.z.    *)
(*                                                                    *)
(*  Axioms: standard classical-Reals only.                            *)
(* ================================================================= *)

From Stdlib Require Import Reals Lra Lia.
Require Import Ell2Zeta CountTwoCollapse.
Open Scope R_scope.
...
Print Assumptions eta_from_zeta.

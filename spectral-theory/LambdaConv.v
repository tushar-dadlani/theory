(* ================================================================= *)
(*                                                                    *)
(*  REDUNDANT.  CVonMangoldtZeta.conv_term_eq is this theorem for       *)
(*  G = gC (the zeta term), with the same proof structure, and          *)
(*  CListSum.Cls_scal_r already exists.  I did not check first.  The    *)
(*  only difference is that lambda_conv is stated for an arbitrary      *)
(*  completely multiplicative G, which is what the twisted case needs,  *)
(*  so it is kept -- but it should be re-proved against                 *)
(*  VonMangoldtGlobal.Lam, not the VonMangoldtReal.Lam used here.       *)

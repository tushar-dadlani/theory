...
(*  and at that delta the two sides meet EXACTLY: the contradiction    *)
(*  needs 108^4 <= 81^3 * 256, and 108^4 = 3^12 * 4^4 = 81^3 * 256.    *)
(*  So no fourth root is ever taken -- the whole optimisation is       *)
(*  rational arithmetic.                                              *)
(*                                                                    *)
(*  That is NOT a coincidence, and the constants are not arbitrary.    *)
(*  Optimising f(d) = (d^p/K)^{1/q} - d gives                          *)
(*                                                                    *)
(*      D = (p/q)^q / K,        U = D (q-p)/p,                        *)
(*      D + U = D q/p = (p/q)^{q-1} / K,                               *)
(*                                                                    *)
(*  so 81/256 is (3/4)^4 and 27/256 is 3^3/4^4.  At K = 1 the exactness *)
(*  reads  (D+U)^q = (p/q)^{q(q-1)}  versus  D^p = (p/q)^{qp}, which   *)
(*  agree IFF q = p + 1.  Checked: (1,2), (2,3), (3,4), (5,6) exact,   *)
(*  (3,5) not.                                                        *)
(*                                                                    *)
(*  And q = p + 1 holds here because p = 3, q = 4 are the exponents of  *)
(*  Mertens 3-4-1, consecutive because                                 *)
(*      2 (1 + cos t)^2 = 3 + 4 cos t + cos 2t.                        *)
(*  So the exactness of this optimisation traces to the consecutiveness *)
(*  of the 3 and the 4 in the inequality that produced it.  Had they   *)
(*  not been consecutive one would be stuck approximating              *)
(*  (delta^3/K)^{1/4}, which in Coq means Rpower and much avoidable    *)
(*  pain.                                                             *)
(*                                                                    *)
...
Proof.
  ...
  (* the exact meeting point: 108^4 = 3^12 * 4^4 = 81^3 * 256,
     which holds because q = p + 1 for the exponents (p,q) = (3,4) of 3-4-1 *)

...
| **`FE.xi_critical_formula`, `Zline`** | — | **absent-in-Coq (overtake)** | **The critical line collapses to one real function of a real variable:** `ξ(1/2 + it) = Z(t)` with `Z(t) = 1/2 − (t²+1/4)·Re M(−3/4 + it/2)`. Two structural facts do it: `mellinTail_conj` (`M(w̄) = conj M(w)`, because `ψ` is real), and the observation that at `s = 1/2 + it` the two exponents in `ξ` are **conjugates of each other**, so the pair sums to `2·Re M` while the prefactor `s(s−1)/2 = −(t²+1/4)/2` is real. Everything imaginary cancels *structurally*, not numerically. |
| `FE.mellinTail_conj`, `Zline_zero_pos`, `xi_eq_zero_of_Zline_eq_zero` | — | **absent-in-Coq** | The conjugation law for `M`; `Z(0) > 0` (transported from `xi_half_re_pos`); and the reduction `Z(t) = 0 → ξ(1/2+it) = 0`. |
| **Costing the numerical route — the honest number** | — | **scope finding** | Existence below height `T` is now exactly `∃ t ∈ (0,T), Z(t) < 0`. The obstruction is quantitative and worth recording: the first zero is at `t ≈ 14.13`, where `Z` has magnitude `~10⁻³` against a leading term of `1/2` — because `Γ(1/4 + it/2)` decays like `e^{−πt/4}`, so `Re M` must be resolved to roughly **five significant figures** to determine a sign. That is a rigorous oscillatory quadrature (`∫₁^∞ u^{−3/4}cos((t/2)log u)ψ(u)du`) with **no interval-arithmetic infrastructure anywhere in this development**. Smaller than building Hadamard, but still a project, not a brick. |
| **Correction to my own estimate** | — | **correction** | Two turns ago I described the Hadamard route as "a large independent development" while listing it as a live option. That understated it: I had not yet confirmed that mathlib **lacks the factorisation theorem entirely** (A20). Both routes to existence are now costed from inspection rather than guessed, and neither is a brick. |

### 3.1g Numeric — rigorous quadrature

| Lean | Coq oracle | Verdict | Note |
|---|---|---|---|
| **`Numeric.norm_integral_sub_midpointSum_le`** | — | **absent-in-Coq (overtake)** | **The composite midpoint rule with an explicit error bound:** `‖∫ₐᵇ f − midpointSum f a b n‖ ≤ L·(b−a)²/n` for an `L`-Lipschitz integrand. Built from `sum_integral_adjacent_intervals` over a uniform partition plus the one-panel primitive. Generic — nothing zeta-specific. **This is `O(1/n)`, not the optimal `O(1/n²)`**: the second-order midpoint bound needs a `C²` hypothesis and is *not* built, so reaching a given accuracy costs quadratically more panels than a sharp implementation would. |
| `Numeric.norm_integral_sub_smul_le`, `..._of_lipschitz` | — | **absent-in-Coq** | The one-panel primitive: `‖∫ₐᵇ f − (b−a)·f(c)‖ ≤ M·(b−a)` when `f` stays within `M` of `f(c)`. Just `norm_integral_le_of_norm_le_const` applied to `f − f(c)`, but everything else is built on it. |
| `Numeric.norm_integral_Ioi_tail_le` | — | **absent-in-Coq** | Truncation: an exponential majorant `K·e^{−u}` past `A` bounds the discarded tail by `K·e^{−A}`. With the A10 decay bound this makes the `∫_A^∞` half of the zeta integral cheap — at `A = 5` the tail is `~10⁻⁷`, well under the `~10⁻⁵` needed. |
| Method note: the `SMul ℝ ℂ` instance blocked `Finset.smul_sum` | — | — | Sixth appearance of this family. `intervalIntegral.integral_const` produces `(b−a) • f c`, but `Finset.smul_sum` would not rewrite `h • ∑ …` against it — so the composite rule could not be assembled in `•` form at all. Fixed by **restating the whole file with `((·:ℝ):ℂ) * ·` instead of `•`**, bridging once at `integral_const` via `congr 1`. Worth noting the shape: the instance mismatch did not merely make a proof awkward, it forced a change to the *statements*. |
| **What is still missing for the zeta application** | — | **scope** | These three give `∫₁^∞ f = finite sum ± two explicit errors`. Applying them to `Re M(−3/4+it/2) = ∫₁^∞ u^{−3/4}cos((t/2)log u)ψ(u)du` needs (i) a **Lipschitz constant `L`** for that integrand — which requires differentiating `ψ` termwise and bounding `ψ′` — and (ii) rigorous evaluation of the resulting finite sum of transcendental terms to `~10⁻⁵`. Neither is built. With the `O(1/n)` bound and `L` plausibly `~10²` on `[1,5]`, hitting `10⁻⁵` would need `n ~ 10⁸` panels — **so the `O(1/n²)` refinement is not optional for this application, it is a prerequisite.** |
